...
local function div_parsestrikethrough(self, strikethrough)
	if not strikethrough then return
	end
	-- 标准格式: strikethrough = 2, #ff0000
	-- (strikethrough_size, strikethrough_color)
	local params = self:split(strikethrough, ",")
	if #params~=2 then
		self:printf("parser <div> property strikethrough error")
		return nil
	end
	params.size = tonumber(params[1]) or 2
	params.color = self:convertColor(params[2]) or cc.c4b(255,255,255,255)
...
local function div_parseglow(self, glow)
	...
	local params = self:split(glow, ",")
	params.size = tonumber(params[1]) or -1
	params.color = self:convertColor(params[2]) or cc.c4b(255,255,255,255)
	return params
end
...
-- 			end
-- 			if glow then
-- 				label:enableGlow(glow_params.color, glow_params.size)
-- 			end
	...
	if self._isAutoAdjustWidth then
		for k, content in ipairs(contents) do
			...
			if label then
				...
				if params.glow then
					label:enableGlow(params.glow_params.color, params.glow_params.size)
				end
				...
				if params.underline then
		        	...
		        	--draw a line
		        	draw:drawLine(cc.p(0,0), cc.p(label:getBoundingBox().width, 0), params.underline_params.color, params.underline_params.size)
				end

				if params.strikethrough then
					local draw = cc.DrawNode:create()
		        	label:addChild(draw, 1)
		        	--draw a line
		        	local y = label:getBoundingBox().height / 2
		        	draw:drawLine(cc.p(0,y), cc.p(label:getBoundingBox().width, y), params.strikethrough_params.color, params.strikethrough_params.size)
				end
		...
		if label then
			...
			if leftSpaceWidth < 0 then 
				...
				if leftRenderer then
					...
					if params.glow then
						leftRenderer:enableGlow(params.glow_params.color, params.glow_params.size)
					end
				end
	            ...
	            if leftRenderer then
	                ...
	                if params.underline then
			        	...
			        	--draw a line
			        	draw:drawLine(cc.p(0,0), cc.p(leftRenderer:getBoundingBox().width, 0), params.underline_params.color, params.underline_params.size)
					...
					if params.strikethrough then
						...
						leftRenderer:addChild(draw, 1)
						--draw a line
						local y = leftRenderer:getBoundingBox().height / 2
						draw:drawLine(cc.p(0,y), cc.p(leftRenderer:getBoundingBox().width, y), params.strikethrough_params.color, params.strikethrough_params.size)
					end
	            end
		        --end
...
return function (self, params, default)
	local curParams = {
		...
		strikethrough_params = div_parsestrikethrough(self, params.strikethrough),
		glow = params.glow,
