...
RichLabel._currentWidth = 0
RichLabel._currentHeight = 0
...
function RichLabel:ctor(params)
	params = params or {}
	local fontName 	= params.fontName or gLocalization:getCurFontName()
	...
	self.tempParsers = {}
	self._hAlignment = cc.TEXT_ALIGNMENT_LEFT
...
function RichLabel:setHorizontalAlignment(hAlignment)
	self._hAlignment = hAlignment
	self:layout()
end
...
function RichLabel:layout()
	if self._isInitFinish == nil then
		return
	end
	...
	for index, line in pairs(alllines) do
		...
		linewidthMap[index] = linewidth
	end
	-- 减去最后多余的一个行间距
	maxheight = maxheight - linespace
	self._currentWidth = math.max(maxwidth, self._maxWidth)
	self._currentHeight = maxheight

	if self._hAlignment == cc.TEXT_ALIGNMENT_CENTER or self._hAlignment == cc.TEXT_ALIGNMENT_RIGHT then
		for index, line in pairs(alllines) do
			local adjust_x = 0
			if self._hAlignment == cc.TEXT_ALIGNMENT_CENTER then
				adjust_x = (self._currentWidth - linewidthMap[index]) / 2
			else
				adjust_x = self._currentWidth - linewidthMap[index]
			end
			for _, node in pairs(line) do
				node:setPositionX(node:getPositionX() + adjust_x)
			end
		end
	end
end
...
function RichLabel:debugDraw(level)
	if self._isInitFinish == nil then
		self:printf("render error, no content")
		return
	end
	level = level or 2
    local containerNode = self._containerNode
	...
	local origin_x = pos_x
	local origin_y = pos_y - labelSize.height
...
function RichLabel:loadLabelParser_(label)
	...
	-- 检测是否存在解析器
	local ok, parser = pcall(require, parserpath)
...
function RichLabel:charsToNodes_(parsedtable, containerNode)
	...
	for index, params in pairs(parsedtable) do
		...
		if parser then
			...
			self:printf("not support label %s", labelname)
		end
	end
	return allnodelist
end
...
function RichLabel:layoutLine_(basepos, line, charspace)
...
function RichLabel:getSprite(filename)
	local pair = self:split(filename, "@")
	if pair and #pair == 2 then
		local plistFile = pair[1]
		local spriteFrameName = pair[2]
...
function RichLabel:getSpine(path)
    local spine = sp.SkeletonAnimation:create(path .. ".json", path .. ".atlas")
    return spine
end
