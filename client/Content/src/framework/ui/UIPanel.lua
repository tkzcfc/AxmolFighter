...
-- @brief 事件派发器
propertyReadOnly(UIPanel, "pEventEmitter")
...
function UIPanel:onDestroy()
	...
	gSysEventEmitter:emit(SysEvent.UI_WILL_DESTROY, self)
...
function UIPanel:onTouchEnded(touch, event)
	if self.bAutoDismiss and
	not self.touchContentViewBeginTag and
	not self:isContainContentView(touch, event) and
	cc.pDistanceSQ(touch:getStartLocation(), touch:getLocation()) < 1000 then
		if self:onClickBlankToClosePreconditions() then
			self:dismiss()
		end
	end
...
function UIPanel:onClickBlankToClosePreconditions()
	return true
end
