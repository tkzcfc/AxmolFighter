-- @Author : github.com/tkzcfc
-- @Date   : 2021-04-17 16:52:49
-- @remark : ccui.ScrollView 扩展

local ScrollViewExAction = require("framework.extend.ScrollViewExAction")
...
function ScrollView:setItemViewModel(itemClass, totalItemNum, itemSize, multiNum, adaptationSize, ...)
    ...
    self.tArgs      = {...}
    self.iMultiNum  = multiNum or self:dynCalculateMultiNum(itemSize)
...
function ScrollView:playItemAction(callback, actionType, ...)
    if actionType == "Scale" then
        ScrollViewExAction.playItemAction_Scale(self, callback, ...)
    elseif actionType == "FlyRightToLeft" then
        ScrollViewExAction.playItemAction_FlyRightToLeft(self, callback, ...)
    elseif actionType == "FlyBottomToTop" then
        ScrollViewExAction.playItemAction_FlyBottomToTop(self, callback, ...)
    else
        ScrollViewExAction.playItemAction_Default(self, callback, ...)
    end
end
...
-- @brief 动态计算一行显示个数
function ScrollView:dynCalculateMultiNum(itemSize)
    local margin = self.hMargin or 0
    local hPadding = self.hPadding or 0
    local viewSize = self:getContentSize().width
    local viewWidth = viewSize - hPadding * 2
    
    local multiNum = viewWidth / (itemSize.width + margin)
    multiNum = math.floor(multiNum)
    if multiNum <= 0 then
        multiNum = 1
    end

    return multiNum, viewWidth / multiNum
end
...
function ScrollView:_updateView(force)
    ...
    for i = #self.tShowItems, 1, -1 do
        ...
        if isVertical then
            viewVal = containerOffset.y + item:getPositionY() - item:getAnchorPointInPoints().y
        else
            viewVal = containerOffset.x + item:getPositionX() - item:getAnchorPointInPoints().x
        end
        ...
        repeat
            for i = 1, self.iMultiNum do
                ...
                if not self:_containItem(index) then
                    ...
                    item.iColumns = i       -- 列数
                    item:setPosition(x + item:getAnchorPointInPoints().x, y + item:getAnchorPointInPoints().y)
...
function ScrollView:_updateContentSize(freezeOffset, itemSize, adaptationSize)
    ...
    self.tItemContentRawSize = clone(itemSize)
...
function ScrollView:_updateCellPos()
    ...
    if ScrollViewDirection.DIR_VERTICAL == self:getDirection() then
        for k,item in pairs(self.tShowItems) do
            ...
            local anchorPointInPoints = item:getAnchorPointInPoints()
            item:setPosition(x + anchorPointInPoints.x, y + anchorPointInPoints.y)
        end
    else
        for k,item in pairs(self.tShowItems) do
...
function ScrollView:_dequeueItem()
    ...
    if #self.tFreeItems <= 0 then
        ...
        item:iItemWillCreate(self.tItemContentSize, self.tItemContentRawSize, self.fItemScale)
    ...
    item:stopAllActionsByTag(ScrollViewExAction.ACTION_TAG)
    item:setOpacity(255)
    return item
end
