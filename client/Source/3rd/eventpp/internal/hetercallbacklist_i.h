...
template <int N,
          ...
          int M>
struct FindPrototypeByCallableFromIndex<N,
                                        ...
                                        typename std::enable_if<(N < M)>::type>
{
    enum
    {
        canInvoke = CanInvoke<Callable, typename ArgTransformer<Args>::type...>::value
    };

    using Prototype = typename std::conditional<
        canInvoke,
        RT(Args...),
        typename FindPrototypeByCallableFromIndex<N + 1, HeterTuple<Others...>, Callable, ArgTransformer, M>::
            Prototype>::type;
...
template <typename PrototypeList_,
          typename Callable,
          template <typename> class ArgTransformer = FindPrototypeDefaultArgTransformer>
...
struct FindPrototypeByArgsFromIndex<N, HeterTuple<RT(Args...), Others...>, InArgs...>
{
    enum
    {
        canInvoke = CanInvoke<RT(Args...), InArgs...>::value
    };
    ...
    enum
    {
        index = canInvoke ? N : FindPrototypeByArgsFromIndex<N + 1, HeterTuple<Others...>, InArgs...>::index
    };
};
